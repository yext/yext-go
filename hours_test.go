...
func TestHoursHelperFromStringPartial(t *testing.T) {
	var tests = []struct {
		Have string
		Want *LocationHoursHelper
	}{
	...
	for i, test := range tests {
		if got, err := LocationHoursHelperFromStringPartial(test.Have); err != nil {
			t.Errorf("Test HoursHelperFromString %d\nGot error: %s", i+1, err)
		} else if !reflect.DeepEqual(got, test.Want) {
			t.Errorf("Test HoursHelperFromString %d\nWanted: %v\nGot: %v", i+1, *test.Want, *got)
		}
...
func TestStructSerializePartial(t *testing.T) {
	var tests = []struct {
		Have *LocationHoursHelper
		Want **Hours
	}{
		{
			...
			Want: NullableHours(&Hours{
				...
				Tuesday:   NullableDayHours(nil),
				Wednesday: NullableDayHours(nil),
				Thursday:  NullableDayHours(nil),
				Friday:    NullableDayHours(nil),
				Saturday:  NullableDayHours(nil),
			}),
		},
		{
			Have: &LocationHoursHelper{
	...
	for i, test := range tests {
		if got := test.Have.StructSerializePartial(); !reflect.DeepEqual(GetHours(got), GetHours(test.Want)) {
			//if want != nil {
			t.Errorf("Test StructSerialize %d.\nWanted: %v\nGot: %v", i+1, GetHours(test.Want), GetHours(got))
			//}
		}
